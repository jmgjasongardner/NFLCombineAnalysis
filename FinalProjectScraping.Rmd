...
combine <- read.csv('Group Project Data.csv')
...
combine = left_join(combine, pb_df, by='Name')

ap_df <- ap_df %>%
  count(vars=c('Pos', 'Player')) %>%
  rename(replace = c('freq' = 'AllPros')) %>%
  rename(replace = c('Player' = 'Name'))
combine = left_join(combine, ap_df, by='Name')
combine['Pos.x'] <- NULL
combine['Pos.y'] <- NULL
...
```{r}
write.csv(combine, 'CombineData.csv')
```
